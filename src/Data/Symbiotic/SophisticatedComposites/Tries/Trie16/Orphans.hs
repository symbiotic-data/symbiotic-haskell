module Data.Symbiotic.SophisticatedComposites.Tries.Trie16.Orphans where

import Data.Symbiotic.SophisticatedComposites.Mappings.Map16 (Map16 (..))
...
instance (Serialize a, Serialize p, Ord p, Serialize (c p a)) => Serialize (MT.MapStep c p a) where
  put (MT.MapStep xs) = put (Map16 xs)
  get = do
    Map16 xs <- get
    pure (MT.MapStep xs)
